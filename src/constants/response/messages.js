// USER

// error login options
export const MSG_NO_USERS = 'No users yet registered.'
export const MSG_NO_USER = 'User not found. Please register.'
export const MSG_INCORRECT_PASSWORD = 'Incorrect password. Please try again'
export const MSG_EMPTY_FIELDS = 'All fields must be filled.'
export const MSG_PASSWORDS_DONT_MATCH = 'Passwords don\'t match.'


// PROJECT
export const MSG_NO_PROJECTS = 'There are no projects'

// FETCH JOB
export const MSG_NO_FETCH_JOBS = 'There are no fetch jobs'

// COLLAB
export const MSG_NO_COLLABS = 'There are no collaborations'

// INFLUENCER
export const MSG_NO_INFLUENCERS = 'There are no influencers'

// FETCH
export const MSG_HASHTAG_MEDIA_SUCCESS = 'success: got users by hashtag'
export const MSG_HASHTAG_MEDIA_ERROR = 'error: didn\'t get users by hashtag'


// NEXT PAGE FETCH
export const COMPLETED_NEXT_PAGE = 'completed: get next page'
