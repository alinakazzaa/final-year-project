// USER

export const SET_USERS_PENDING = 'SET_USERS_PENDING'
export const SET_USERS_ERROR = 'SET_USERS_ERROR'
export const SET_USERS_SUCCESS = 'SET_USERS_SUCCESS'

// PROJECT

export const SET_PROJECTS_PENDING = 'SET_PROJECTS_PENDING'
export const SET_PROJECTS_SUCCESS = 'SET_PROJECTS_SUCCESS'
export const SET_PROJECTS_ERROR = 'SET_PROJECTS_ERROR'

// FETCH JOB

export const SET_FETCH_JOBS_PENDING = 'SET_FETCH_JOBS_PENDING'
export const SET_FETCH_JOBS_SUCCESS = 'SET_FETCH_JOBS_SUCCESS'
export const SET_FETCH_JOBS_ERROR = 'SET_FETCH_JOBS_ERROR'

// INFLUENCER

export const SET_INFLUENCERS_SUCCESS = 'SET_INFLUENCERS_SUCCESS'
export const SET_INFLUENCERS_PENDING = 'SET_INFLUENCERS_PENDING'
export const SET_INFLUENCERS_ERROR = 'SET_INFLUENCERS_ERROR'


// MEDIA FETCH

export const GET_MEDIA_SUCCESS = 'GET_MEDIA_SUCCESS'
export const GET_MEDIA_ERROR = 'GET_MEDIA_ERROR'
export const GET_MEDIA_PENDING = 'GET_MEDIA_PENDING'
export const COMPLETED_GET_USERS = 'COMPLETED_GET_USERS'
export const COMPLETED_FETCH = 'COMPLETED_FETCH'


// USER FETCH
export const GET_USER_PENDING = 'GET_USER_PENDING'
export const GET_USER_SUCCESS = 'GET_USER_SUCCESS'
export const GET_USER_ERROR = 'GET_USER_ERROR'

// COLLAB

export const SET_COLLABS_PENDING = 'SET_COLLABS_PENDING'
export const SET_COLLABS_SUCCESS = 'SET_COLLABS_SUCCESS'
export const SET_COLLABS_ERROR = 'SET_COLLABS_ERROR'

export const GET_USER_MEDIA_PENDING = 'GET_USER_MEDIA_PENDING'
export const GET_USER_MEDIA_SUCCESS = 'GET_USER_MEDIA_SUCCESS'
export const GET_USER_MEDIA_ERROR = 'GET_USER_MEDIA_ERROR'

export const GET_COLLAB_INFLUENCER_SUCCESS = 'GET_COLLAB_INFLUENCER_SUCCESS'
export const GET_COLLAB_INFLUENCER_ERROR = 'GET_COLLAB_INFLUENCER_ERROR'
export const REMOVE_INFLUENCER_FROM_FETCH_JOB = 'REMOVE_INFLUENCER_FROM_FETCH_JOB'

